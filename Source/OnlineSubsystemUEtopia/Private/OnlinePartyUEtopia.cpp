...
bool FOnlinePartyUEtopia::JoinParty(const FUniqueNetId& LocalUserId, const IOnlinePartyJoinInfo& OnlinePartyJoinInfo, const FOnJoinPartyComplete& Delegate /*= FOnJoinPartyComplete() */)
{
	...
	// kick off http request to create the party
	HttpRequest->OnProcessRequestComplete().BindRaw(this, &FOnlinePartyUEtopia::JoinParty_HttpRequestComplete, Delegate);
	FString TeamsUrlHardcoded = TEXT("https://ue4topia.appspot.com/_ah/api/teams/v1/join");
...
void FOnlinePartyUEtopia::JoinParty_HttpRequestComplete(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded, FOnJoinPartyComplete Delegate)
{
	bool bResult = false;
	FString ResponseStr, ErrorStr;
	FString PartyIdStr = "invalid";
		...
		HttpResponse.IsValid())
	{
		...
		if (EHttpResponseCodes::IsOk(HttpResponse->GetResponseCode()))
		{
			UE_LOG(LogOnline, Verbose, TEXT("JoinParty_HttpRequestComplete. url=%s code=%d response=%s"),
				*HttpRequest->GetURL(), HttpResponse->GetResponseCode(), *ResponseStr);
	...
	if (!ErrorStr.IsEmpty())
	{
		UE_LOG(LogOnline, Warning, TEXT("Join Party request failed. %s"), *ErrorStr);
	}
...
void FOnlinePartyUEtopia::RespondToQueryJoinability(const FUniqueNetId& LocalUserId, const FOnlinePartyId& PartyId, const FUniqueNetId& RecipientId, bool bCanJoin, int32 DeniedResultCode)
{
	return;
}
