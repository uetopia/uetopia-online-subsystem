...
private:
	...
	class FUEtopiaPermissionsJson : public FOnlineJsonSerializable
	{
		public:
			class FUEtopiaPermissionJson :
			...
			/** Main error body */
			TArray<FUEtopiaPermissionJson> Permissions;

			BEGIN_ONLINE_JSON_SERIALIZER
				ONLINE_JSON_SERIALIZE_ARRAY_SERIALIZABLE(PERM_JSON_PERMISSIONS, Permissions, FUEtopiaPermissionJson);
			END_ONLINE_JSON_SERIALIZER
	};
...
*/
class FOnlineSharingUEtopiaCommon : public IOnlineSharing
{
...
public:
	...
	*/
	FOnlineSharingUEtopiaCommon(FOnlineSubsystemUEtopia* InSubsystem);

	/**
	* Default destructor
	*/
	virtual ~FOnlineSharingUEtopiaCommon();

protected:

	/** Parent subsystem */
	FOnlineSubsystemUEtopia* Subsystem;
	/** Permissions request URL */
	FString PermissionsURL;
	/** Current state of granted/declined permissions */
	FUEtopiaPermissions CurrentPermissions;
...
typedef TSharedPtr<FOnlineSharingUEtopiaCommon, ESPMode::ThreadSafe> FOnlineSharingUEtopiaCommonPtr;
