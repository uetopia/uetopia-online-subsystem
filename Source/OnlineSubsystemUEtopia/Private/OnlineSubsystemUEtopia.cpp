...
{	
	...
	ConnectionThread = FSIOLambdaRunnableUEtopia::RunLambdaOnBackGroundThread([&, SafeHeaders, SafeQuery]
	{
		...
		// listen for incoming team invitations
		OnNativeEvent(FString("team_invitation"), [](const FString& Event, const TSharedPtr<FJsonValue>& Message)
		{
			UE_LOG_ONLINE(Display, TEXT("team_invitation incoming"));
		}, FString("/"));

		// listen for incoming team change notifications
		OnNativeEvent(FString("team_changed"), [](const FString& Event, const TSharedPtr<FJsonValue>& Message)
		{
			UE_LOG_ONLINE(Display, TEXT("team_changed incoming"));
		}, FString("/"));
