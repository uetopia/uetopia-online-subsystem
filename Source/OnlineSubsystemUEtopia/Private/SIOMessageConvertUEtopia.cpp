...
TSharedPtr<FJsonValue> USIOMessageConvertUEtopia::ToJsonValue(const sio::message::ptr& Message)
{
	if (Message == nullptr)
	{
...
sio::message::ptr USIOMessageConvertUEtopia::ToSIOMessage(const TSharedPtr<FJsonValue>& JsonValue)
{
...
std::string USIOMessageConvertUEtopia::StdString(FString UEString)
{
	return std::string(TCHAR_TO_UTF8(*UEString));
}

FString USIOMessageConvertUEtopia::FStringFromStd(std::string StdString)
{
	return FString(UTF8_TO_TCHAR(StdString.c_str()));
}

std::map<std::string, std::string> USIOMessageConvertUEtopia::JsonObjectToStdStringMap(TSharedPtr<FJsonObject> InObject)
{
	...
	for (auto Pair : InObject->Values)
	{
		...
		{
			HeadersMap[USIOMessageConvertUEtopia::StdString(Pair.Key)] = USIOMessageConvertUEtopia::StdString(Value->AsString());
		}
	}
